...
import { Lang, CVContact } from './types'
...
function Contacts({ contact }: { contact: CVContact }) {
  return (
...
export default function App() {
  ...
  const enPdf = pdfUrl('en')
  const dePdf = pdfUrl('de')
  ...
  return (
    <>
      ...
      <main className="main-content">
        ...
        <Contacts contact={cv.contact.content} />

        <section className="cv-section">
          <div className="section-header">
            <h2 className="section-title">{cv.profile.title}</h2>
            <div className="pdf-links">
              {enPdf && <a className="ctrl-btn" href={enPdf} download>en.pdf</a>}
              {dePdf && <a className="ctrl-btn" href={dePdf} download>de.pdf</a>}
            </div>
          </div>
